...
This release updates **Virtual Wipe** from **2.7.0** to **2.8.0**, correcting the SSD/TRIM code path and restoring an honest sanitization advisory.

## 🔧 Correctness Fixes (vwipe.c)

### 1. TRIM / Secure Discard Actually Executes
* **Problem (≤2.7.0):** `attempt_trim()` opened a *regular file* descriptor and called `ioctl(fd, BLKDISCARD, {0,0})`. This was a double no-op: `BLKDISCARD` only operates on block-device nodes (it returns `ENOTTY`/`EINVAL` on a file), and a `{start:0, len:0}` range discards zero bytes. No TRIM ever occurred, and the result was unchecked.
* **Fix (2.8.0):** `attempt_trim()` now dispatches by target type:
  * **Block device** → `BLKSECDISCARD` (fallback `BLKDISCARD`) over the full device using the size from `BLKGETSIZE64`.
  * **File / directory** → filesystem-wide `FITRIM` on the containing filesystem.
* Call sites were reordered so TRIM runs **after** `unlink()`/`rmdir()` (FITRIM only reclaims already-free space). Directory wipe now trims the parent filesystem after removal.

### 2. Honest Storage Advisory Restored
* New `storage_advisory()` detects SSD/NVMe targets (via `/sys/dev/block/<maj>:<min>/queue/rotational`) and journaling/CoW filesystems (via `statfs` `f_type`), logging warnings that software overwrite cannot guarantee erasure on flash, and recommending hardware Secure Erase / crypto-erase. Invoked at the start of file and free-space wipes.

### 3. Free-Space Safety Margin
* **Problem (≤2.7.0):** `SAFE_ZONE_BYTES` was `0`, so Free Space Wipe filled the filesystem to 100%, risking `ENOSPC` starvation of other processes on a live system.
* **Fix (2.8.0):** `SAFE_ZONE_BYTES` is now 256 MB and is subtracted from the wipe target, with an abort if free space is below the margin.

### 4. Documentation Honesty
* The in-app TRIM feature description and the README no longer over-promise on flash media; SSD over-provisioning limits are stated explicitly.
